...
class TradeCardViewer extends React.Component {
  constructor(props) {
    ...
    this.gridDimensions = {
      x: this.gridDimensions[0] + this.gridDimensions[1] + 1,
      y: this.gridDimensions[2] + this.gridDimensions[3] + 1,
    };
  ...
  createTableRow(rowArrayElement) {
    return (
      <TableRow key={"row" + rowArrayElement} id={"row" + rowArrayElement}>
        {ZerotoNumArray(this.gridDimensions.y).map((x, i) => {
          return this.createTableElement(i, rowArrayElement);
        })}
      </TableRow>
    );
  }
  createTableElement(columnArrayElement, rowIndex) {
    return (
      <TableCell
        className="getRidOfBottomLinePlease"
        key={"" + rowIndex + columnArrayElement}
        id={"" + rowIndex + columnArrayElement}
      ></TableCell>
    );
  }
  populateElements() {
    for (let imageNode of this.tradecardTree.postOrderTraversal()) {
      ReactDOM.render(
        <CardImageComponent
          ...
          center={this.center}
          toggle={this.toggleCardsOnOff}
          id={
            "node" +
            (this.center.y - imageNode.location.y) +
            (this.center.x + imageNode.location.x)
          }
          node={imageNode}
        />,
  ...
  closeAllChildCards(node) {
    //If you are getting rid of cards, you can take advantage of the fact that only closing needs you to do recursion on the later cards. Whereas adding only adds one at a time.
    document.getElementById(JSON.stringify(node.location)).style.display =
      "none";
    for (let child of node.children.filter((x) => x !== null)) {
      this.closeAllChildCards(child);
    }
...
function CardImageComponent(props) {
  let buttonArray = props.node.children.map((x) => {
    return x !== null;
  });
  return (
    <div
      id={JSON.stringify(props.node.location)}
      className={
        props.node.location.x === 0 && props.node.location.y === 0
          ? ""
          : "child"
      }
    >
        ...
        <tbody>
          <tr>
            <th
              colSpan={3}
              onClick={() => {
                props.toggle(props.node.children[0]);
              }}
              className={buttonArray[0] ? "topArea" : ""}
            >
              <p></p>
            </th>
          </tr>
          <tr>
            <th
              onClick={() => {
                props.toggle(props.node.children[2]);
              }}
              className={buttonArray[2] ? "leftArea" : "horizontal"}
            ></th>
            <th>
            ...
            <th
              onClick={() => {
                props.toggle(props.node.children[3]);
              }}
              className={buttonArray[3] ? "rightArea" : "horizontal"}
            ></th>
          </tr>
          <tr>
            <th
              colSpan={3}
              onClick={() => {
                props.toggle(props.node.children[1]);
              }}
              className={buttonArray[0] ? "bottomArea" : ""}
            >
              <p></p>
            </th>
          </tr>
